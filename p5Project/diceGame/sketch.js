...
function setup() {
  ...
  random1x = random(canvasWidth * 0.22, canvasWidth);
...
function draw() {
  ...
  stroke(0,0,0,0);
  fill('#010626');
  rect(0, 0, (canvasWidth/2)-300, canvasHeight);
  ...
  push();
  translate(width * 0.14, height * 0.65);
  polygon(0, 0, 120, 6);
  pop();
...
function mousePressed() {
  ...
  random1y = random(height);
   ...
   // Check if the coordinates are within the restricted area
   if (random1x < (canvasWidth / 2) - 300 && random1y < canvasHeight) {
    return; // If they are, don't draw anything and return immediately
  }
